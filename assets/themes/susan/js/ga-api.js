...
module.exports = function(args, callback, settings) {
  if (settings) {
    ...
    cacheDir =
      typeof settings.cacheDir !== 'undefined' ? settings.cacheDir : cacheDir
    concurrentLimit =
      typeof settings.concurrentLimit !== 'undefined'
        ? settings.concurrentLimit
        : concurrentLimit
    concurrentDelay =
      typeof settings.concurrentDelay !== 'undefined'
        ? settings.concurrentDelay
        : concurrentDelay
    concurrentMaxRetry =
      typeof settings.concurrentMaxRetry !== 'undefined'
        ? settings.concurrentMaxRetry
        : concurrentMaxRetry
  }

  var jwt = new googleapis.auth.JWT(args.email, args.key, null, [
      'https://www.googleapis.com/auth/analytics.readonly'
    ]),
    oauth2Client = new OAuth2(args.clientId, null, 'postmessage'),
    sessionFile =
      cacheDir + 'ga-runner-' + args.email.replace(/[^a-zA-Z\-]/gi, '_'),
      ...
      if (cache) {
        ...
        fs.readFile(fileName, 'utf8', function(err, data) {
          if (!err) {
            ...
            if (
              stats.isFile() &&
              stats.birthtime >= new Date().getTime() - cache
            ) {
